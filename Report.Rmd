...
```{r}
fires_a_2 <- read_rds(here::here("data", "fires_clean_all.rds"))

MA <- NULL
CMA <- NULL

for(i in 1:238){
  MA[i+1] <- (fires_a_2$fires[i] + fires_a_2$fires[i+1]
              + fires_a_2$fires[i+2])/3
}
...
for(i in 3:238){
  n_alt[i] <- fires_a_2$fires[i]/CMA[i]
}
...
month_2[which(month_2 == 1)] <- "January"
month_2[which(month_2 == 2)] <- "February"
month_2[which(month_2 == 3)] <- "March"
month_2[which(month_2 == 4)] <- "April"
month_2[which(month_2 == 5)] <- "May"
month_2[which(month_2 == 6)] <- "June"
month_2[which(month_2 == 7)] <- "July"
month_2[which(month_2 == 8)] <- "August"
month_2[which(month_2 == 9)] <- "September"
month_2[which(month_2 == 10)] <- "October"
month_2[which(month_2 == 11)] <- "November"
month_2[which(month_2 == 12)] <- "December"


fires_b <- as.data.frame(cbind(fires_a_2, month_2, MA, CMA, n_alt))
=======
month_nr <- NULL
for(i in 1:239){
  month_nr[i] <- i%%12
...
>>>>>>> 2a008cf805fe6969ba2642da33dce1bcbf9555f7
head(fires_b)
```
...
fires_decomp_models <- fires_decomp %>% 
  rename(fires_original = fires, fires_model = random) %>% 
  as_tsibble() %>% 
  fabletools::model(
    arima = ARIMA(fires_model), 
    snaive = SNAIVE(fires_model)
  )
